...
function checkWinner(board) {
    ...
    for (let i = 0; i < board.length; i++) {
        if (board[i][0] !== '' && board[i][0] === board[i][1] && board[i][1] === board[i][2]) {
            return [[i, 0], [i, 1], [i, 2]]; // Return winning cell positions
        }
    }
    ...
    for (let j = 0; j < board[0].length; j++) {
        if (board[0][j] !== '' && board[0][j] === board[1][j] && board[1][j] === board[2][j]) {
            return [[0, j], [1, j], [2, j]]; // Return winning cell positions
        }
    }

    // Check diagonals
    if (board[0][0] !== '' && board[0][0] === board[1][1] && board[1][1] === board[2][2]) {
        return [[0, 0], [1, 1], [2, 2]]; // Return winning cell positions
    }
    if (board[0][2] !== '' && board[0][2] === board[1][1] && board[1][1] === board[2][0]) {
        return [[0, 2], [1, 1], [2, 0]]; // Return winning cell positions
    }
...
function PrintXO() {
    ...
    // let gameBoard.currentPlayer = gameBoard.gameBoard.currentPlayer;
...
function checkWin(gameBoard, board) {
    ...
    if (winningPositions) {
        ...
        whoWon.textContent = `Player ${gameBoard.currentPlayer === 'X' ? 'O' : 'X'} Won`
    }
}
...
function Selectors(row, col) {
    ...
    const winCells = document.querySelector(`.cell[data-row="${row}"][data-col="${col}"]`);
